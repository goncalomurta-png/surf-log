...
Skills com `val = null` são **excluídas** da média ponderada (numerador e denominador reduzidos pelo mesmo peso). `skills_hist` é a fonte primária para o cálculo de progressão: `[leitura, takeoff, paddle, manobras, equilibrio, posicionamento]` — coincide com `skills.*.val` nas sessões completas; nas sessões antigas sem dict `skills`, contém os valores reais observados. `skills.*.val` é a fonte qualitativa (nota + avaliação por skill) e tem precedência quando presente.
